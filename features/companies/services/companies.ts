...
export async function updateCompany(
  ...
  const { data: updatedCompany, error } = await query.update(updates).eq('id', companyId).select().single();
 ...
 * 기업 활성화/비활성화
 */
export async function toggleCompanyStatus(companyId: string, isActive: boolean): Promise<Company> {
