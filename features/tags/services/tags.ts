...
export async function getTag(tagName: string): Promise<Tag | null> {
  ...
  const { data: tag, error } = await supabase.from('tags').select('*').eq('name', tagName).single();
...
export async function createTag(name: string, category?: string): Promise<Tag> {
  ...
  const { data: tag, error } = await (supabase.from('tags') as any).insert([{ name, category }]).select().single();
 ...
 * 여러 태그 생성/업데이트
 */
export async function upsertTags(tagNames: string[], category?: string): Promise<Tag[]> {
  const tags: Tag[] = [];
