...
import { PostWithCompany, Post } from '@/supabase/types.supabase';
...
export async function fetchPosts(options: FetchPostsOptions = {}): Promise<PostsResult> {
  ...
  const { page = 1, limit = 20, search = '', tags = [], companyId = '' } = options;
...
export async function savePost(post: Omit<Post, 'id' | 'created_at' | 'updated_at'>): Promise<Post | null> {
  ...
  const { data: newPost, error } = await (supabase.from('posts') as any).insert([post]).select().single();
 ...
 * 게시글 벌크 저장
 */
export async function savePosts(posts: Omit<Post, 'id' | 'created_at' | 'updated_at'>[]): Promise<{
  created: Post[];
  duplicates: string[];
