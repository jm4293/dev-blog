 ...
 * 사용자의 북마크된 게시글 조회
 */
export async function getUserBookmarks(userId: string, page: number = 1, limit: number = 20): Promise<BookmarksResult> {
  const supabase = getSupabaseServerClient();

  const offset = (page - 1) * limit;

  // 전체 북마크 수
  const { count } = await supabase.from('bookmarks').select('id', { count: 'exact', head: true }).eq('user_id', userId);
