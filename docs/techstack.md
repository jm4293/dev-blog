...
- **Tailwind CSS 3.4.0**
  ...
  - CSS 변수 기반 모노크롬 팔레트 (`--background`, `--foreground`, `--muted` 등)
  - Tailwind `gray` 팔레트를 pure neutral(`hsl(0, 0%, X%)`)로 재정의 (blue tint 제거)
  - 글래스모피즘 유틸리티 클래스 (`glass`, `glass-card`, `glass-modal`, `glass-sidebar`)

- **shadcn/ui**
  - 설치된 컴포넌트: `button`, `badge`, `separator`, `tooltip`
  - Radix UI 기반 접근성 완비 컴포넌트
  - `components/ui/` 에 위치
...
- **lucide-react**: 아이콘
- **shadcn/ui**: 접근성 완비 UI 컴포넌트 (`components/ui/`)
- **GSAP**: 고성능 애니메이션 (데스크탑 사이드바 hover expand/collapse)
