...
10. [Push 알림](#10-push-알림-기능)
11. [ISR 캐시 갱신](#11-isr-incremental-static-regeneration)
12. [UI/UX](#12-uiux)
13. [태그/회사 랜딩 페이지 (SEO)](#13-태그회사-랜딩-페이지-seo)
14. [트렌딩 (이번 주 인기 글)](#14-트렌딩-이번-주-인기-글)
15. [주간 인기글](#15-주간-인기글)
...
- **스케줄**: 6시간마다, 하루 4회 (KST 03:00, 09:00, 15:00, 21:00 / UTC 00:00, 06:00, 12:00, 18:00)
...
- `notification_preferences.subscribed_tags` / `subscribed_company_ids`에 관심 태그/회사 저장
- **빈 배열(기본값) = 전체 새 글 알림**, 값이 있으면 매칭되는 글이 있을 때만 발송
- 수집 스크립트가 새 글의 태그/회사 정보를 `/api/notifications/send`에 전달 → 유저별 매칭 수 계산 후 개인화 메시지 발송
- 설정 UI: 프로필 > 알림 설정 > 관심사 설정 (`features/notifications/ui/notification-interests.tsx`)

### DB 테이블

- `push_subscriptions`: 장치별 구독 정보
- `notification_preferences`: 유저별 전체 알림 설정 + 관심 태그/회사
...
검색 엔진 색인 확대를 위한 목록형 랜딩 페이지. 게시글 상세 페이지 금지(저작권) 제약을 지키면서 색인 가능한 URL을 늘린다.

### 라우트

- `/tags` — 태그 허브 (카테고리별 그룹)
- `/tags/[slug]` — 태그별 글 목록 (예: `/tags/react`)
- `/companies` — 기업 허브
- `/companies/[slug]` — 기업별 글 목록 (예: `/companies/toss`)
...
`/posts` 첫 페이지(필터 없음) 상단에 최근 7일 인기 글 4개 노출.

### 집계 방식

- `posts.bookmark_count`: bookmarks 테이블 트리거로 동기화 (RLS 때문에 집계 컬럼 필요)
- `posts.view_count`: "전체 읽기" 클릭 시 `increment_post_view` RPC로 증가 (SECURITY DEFINER)
- 정렬: 북마크 수 → 조회수 → 최신순 (최근 7일에 글이 없으면 30일로 확장)
- 마이그레이션: `018_add_post_engagement.sql`
...
주차별 인기 글 TOP 10 + 회사/태그 통계 페이지. SEO 콘텐츠 누적 + 뉴스레터 본문 재활용 목적.

### 라우트

- `/digest` — 최근 12주 목록
- `/digest/[week]` — 주차별 인기글 (ISO 주차, 예: `/digest/2026-W27`)

### 구현 방식

- ISO 주차 유틸: `utils/week.ts` (`parseISOWeekString`, `getRecentWeeks` 등)
- 집계: 해당 주 발행 글 전체 조회 후 인기 점수(북마크×3 + 조회수) 정렬 (`features/digest/services/fetch-digest.ts`)
- ISR `revalidate = 3600`, 최근 8주 `generateStaticParams`, sitemap 포함
